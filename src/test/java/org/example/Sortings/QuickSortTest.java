...
class QuickSortTest {
    ...
    void testSort_IntegerList() {
        ...
        List<Integer> sorted = quickSort.sort(list, Comparator.naturalOrder());
    ...
    void testSort_StringList() {
        ...
        List<String> sorted = quickSort.sort(list, Comparator.naturalOrder());
    ...
    void testSort_ReverseOrder() {
        ...
        List<Integer> sorted = quickSort.sort(list, Comparator.reverseOrder());
        assertEquals(Arrays.asList(5, 4, 3, 2, 1), sorted);
    }

    // Прямой вызов алгоритма быстрой сортировки
    @Test
    void testQuickSortTask_DirectCall() {
        List<Integer> list = Arrays.asList(5, 2, 8, 1, 9);
        QuickSort.QuickSortTask<Integer> task = new QuickSort.QuickSortTask<>(list, Comparator.naturalOrder());
        List<Integer> sorted = task.compute();
        assertEquals(Arrays.asList(1, 2, 5, 8, 9), sorted);
    }
    ...
    void testPerformance_BestVsWorstCase() {
        ...
        // Лучший случай - случайные данные (хорошее разделение)
        List<Integer> bestCase = new ArrayList<>();
        for (int i = 0; i < size; i++) {
            bestCase.add((int) (Math.random() * size));
        }

        // Худший случай - уже отсортированный список (плохое разделение с первым элементом как pivot)
        ...
        System.out.printf("Лучший случай (случайные данные): %d нс%n", bestTime);
        System.out.printf("Худший случай (отсортированные данные): %d нс%n", worstTime);
    ...
    private long measureSortTime(List<Integer> list) {
        ...
        quickSort.sort(new ArrayList<>(list), Comparator.naturalOrder());
